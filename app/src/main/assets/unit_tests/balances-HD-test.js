...
var jaxx;
(function (jaxx) {
    var BalancesTest = (function () {
        function BalancesTest() {
            var _this = this;
            this.balanceDOGE = 'https://api.jaxx.io/api/doge/balance/';
            this.addressesDOGE = [
            ...
            this.balanceLTC = 'https://api.jaxx.io/api/ltc/balance/';
            this.addressesLTC = [
            ...
            setInterval(function () { return _this.downloadBalances(); }, 2000);
        }
        BalancesTest.prototype.logBalances = function (res) {
            for (var str in res) {
                var item = res[str];
                var confirmed = item.confirmed;
                var prev = this.previous[str].confirmed;
        ...
        BalancesTest.prototype.downloadBalances = function () {
            var _this = this;
            var url = this.balanceLTC + this.addressesLTC;
            console.log(url);
            $.getJSON(url).done(function (res) {
                ...
                for (var str in res) {
                    ...
                    var unconfirmed = item.unconfirmed;
                    amount += +unconfirmed.amount;
                }
                if (_this.currentbalance === -1) {
                    _this.currentbalance = amount;
                    return;
                }
                if (_this.currentbalance !== amount) {
                    console.error(' balances not equal old: ' + _this.currentbalance + '  new: ' + amount);
                    if (_this.previous)
                        _this.logBalances(res);
                    _this.currentbalance = amount;
                }
                _this.previous = res;
                console.log(res);
    ...
    jaxx.BalancesTest = BalancesTest;
})(jaxx || (jaxx = {}));
$(document).ready(function () {
    var test = new jaxx.BalancesTest();
});
//# sourceMappingURL=balances-HD-test.js.map