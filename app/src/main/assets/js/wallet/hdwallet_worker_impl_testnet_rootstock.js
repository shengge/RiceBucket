...
var HDWalletWorkerTestnetRootstock = function() {
    this._doDebug = false;
    this._batchSizeGather = 30;
    this._batchSizeDetails = 30;
    this._batchSizeBalances = 30;

    this._coinName = "RSK Testnet";
...
HDWalletWorkerTestnetRootstock.networkParams = { // Might be legacy
    static_relay_url: "http://54.67.34.99/api/eth",
...
HDWalletWorkerTestnetRootstock.relayManagerParams = {
    isSupported: false,
    implementationFileName: "",
}

HDWalletWorkerTestnetRootstock.prototype.initialize = function(workerManager) {
    this._workerManager = workerManager;
}

HDWalletWorkerTestnetRootstock.prototype.log = function(logString) {
    if (this._doDebug === false) {
        return;
    }

    var args = [].slice.call(arguments);
    args.unshift('RootstockTestnetWorkerLog:');
    console.log(args);
}

HDWalletWorkerTestnetRootstock.prototype.batchScanBlockchain = function(addresses) {
    var self = this;
    ...
    while (addresses.length) {
        ...
        if (batch.length === this._batchSizeGather || addresses.length === 0) {
            ...
            var addressParam = batch.join(HDWalletWorkerTestnetRootstock.networkParams.joinParameters);

            //            this.log("ethereum classic :: requesting :: " + addressParam);

            var requestURL = HDWalletWorkerTestnetRootstock.networkParams.static_relay_url + HDWalletWorkerTestnetRootstock.networkParams.gather_tx + addressParam + HDWalletWorkerTestnetRootstock.networkParams.gather_tx_append;
...
HDWalletWorkerTestnetRootstock.prototype._populateHistory = function(processorData, passthroughParams) {
    if (!processorData) {
...
HDWalletWorkerTestnetRootstock.prototype._lookupTransactions = function(allTxList, callback) {
    var self = this;

    console.log("ethereum classic :: lookupTransactions :: allTxList :: " + JSON.stringify(allTxList));
    ...
    while (txBatch.length) {
        ...
        if (batch.length === this._batchSizeDetails || txBatch.length === 0) {
            ...
            var requestURL = HDWalletWorkerTestnetRootstock.networkParams.static_relay_url + HDWalletWorkerTestnetRootstock.networkParams.tx_details + addressParam + HDWalletWorkerTestnetRootstock.networkParams.tx_details_append;
...
HDWalletWorkerTestnetRootstock.prototype._populateTransactions = function(transactions, passthroughParams) {
    if (!transactions) {
...
HDWalletWorkerTestnetRootstock.prototype._updateTransactions = function(transactions, associativeAddressList) {
        ...
        for (var addressIdx in associativeAddressList[txid]) {
            ...
            if (!existingTransaction) {
                ...
                if (addressInfo !== null) {
                    ...
                    console.log("[pre] ethereum :: accountIndex :: " + addressInfo.index + " :: balance :: " + addressInfo.accountBalance);
...
HDWalletWorkerTestnetRootstock.prototype._populateHistoryNotWorking = function(processorData, passthroughParams) {
    if (!processorData) {
        this.log("hdwalletworker :: " + this._coinName + " :: _populateHistory :: error :: address :: " + passthroughParams[0] + " :: is not returning success :: addressData :: " + JSON.stringify(addressData));
...
HDWalletWorkerTestnetRootstock.prototype.updateBalancesEthereum = function() {
    ...
    while (addressesToCheck.length) {
        ...
        if (batch.length === this._batchSizeBalances || addressesToCheck.length === 0) {
            ...
            var requestURL = HDWalletWorkerTestnetRootstock.networkParams.static_relay_url + HDWalletWorkerTestnetRootstock.networkParams.multi_balance + addressParam + HDWalletWorkerTestnetRootstock.networkParams.multi_balance_append;

            console.log("HDWalletWorkerTestnetRootstock :: checking :: " + batch + " :: " + batch.length + " :: requestURL :: " + requestURL);
...
HDWalletWorkerTestnetRootstock.prototype._updateBalancesEthereum = function(processorData, passthroughParams) {
    ...
    if (!processorData) {// || data.status !== 'success') {
        this.log("HDWalletWorkerTestnetRootstock._updateBalancesEthereum :: error :: processorData is incorrect :: " + JSON.stringify(processorData) + " :: passthroughParams :: " + JSON.stringify(passthroughParams));
        return;
    }

    //    var inputAddresses = passthroughParams.split(HDWalletWorkerTestnetRootstock.networkParams.joinParameters);

    //    this.log("HDWalletWorkerTestnetRootstock :: updated balances :: " + JSON.stringify(processorData));
...
HDWalletWorkerTestnetRootstock.prototype.updateBalances = function() {
    this.updateBalancesEthereum();
}

HDWalletWorkerTestnetRootstock.prototype.performRecheck = function() {
    this.updateBalances();
}