...
var JaxxCryptoController = (function () {
    ...
    function JaxxCryptoController(pouch, config) {
        ...
        this.intervalCurrentBalanceCheck = 5000;
    ...
    JaxxCryptoController.prototype.processConfig = function (config) {
        this.config = config;
        this.options = config.options[this._coinType] || {};
        var defaults = config.defaults;
        for (var str in defaults)
            if (typeof this.options[str] === 'undefined')
                this.options[str] = defaults[str];
    };
    JaxxCryptoController.prototype.init = function () {
        ...
        // console.log(Date.now() - starttime);
        this.transactionsUpdater = new jaxx.TransactionsUpdater(this, { updateTimeout: 10000, confirmations: 15 });
    ...
    JaxxCryptoController.prototype.sendAmountToAddress = function (address, amount, addressChange) {
        console.error('it is for testing');
        return;
        /* var transaction:VOSendTransaction = new VOSendTransaction();
         transaction.toAddress = address
         transaction.balancesReceive = this._db.getBalancesReceiveNot0WithSpend();

         transaction.balancesChange = this._db.getBalancesNot0Change();

         transaction.changeAddress = this._db.getCurrentAddressChange();
         transaction.changeAddress = addressChange;
         transaction.amount = amount;
         //transaction.fee = fee;
         // transaction.gasLimit = gasLimit;

         this._accountService.sendTransaction2(transaction).done((res:VOSendTransactionResult) =>{

         var sentBalances:VOBalance[] = res.balancesSent;
         // sentBalances.forEach(b =>b.balance= -b.spent);
         /////////////////////////////////////////////////////////////////
         this._db.addBalancesSpent(sentBalances);

         this._pouch._notify('sent transaction');
         // var balances:VOBalance[] this._db.get

         console.warn('sendTransactin2 result  ',res);

         });*/
    };
    /*private _onDownloadedNewTransactionsChange(transactions:VOTransaction[]):void{
     var newtransactions:VOTransaction[]  = this._db.updateTransactionsChange(transactions);
     if(newtransactions.length) this.onNewTransactions(newtransactions);
     }

     private _onDownloadedNewTransactionsReceive(transactions:VOTransaction[]):void{
     var newtransactions:VOTransaction[]  = this._db.updateTransactionsReceive(transactions);
     if(newtransactions.length) this.onNewTransactions(newtransactions);
     }
     */
    ...
    JaxxCryptoController.prototype.deactivate = function () {
        console.log(this.name + ' deactivating was busy ' + this.isBusy);
        if (this.isBusy) {
    ...
    JaxxCryptoController.prototype.getNonceForFirstAddress = function () {
        return this.transactionController.getNonceForAddress(this.getAddressRecieve(0));
    };
    ...
    JaxxCryptoController.prototype.getAddressRecieve = function (i) {
    ...
    JaxxCryptoController.prototype.startBalancesCheck = function () {
        ...
        if (this.intervalBalancesCheck === 0) {
            ...
            this.intervalBalanceReceive = setInterval(function () { return _this.checkBalanceCurrentReceive(); }, this.intervalCurrentBalanceCheck);
    ...
    JaxxCryptoController.prototype.onCurrentReceiveAddressGotBalance = function (balance) {
        ...
        this.downloadAllBalances(function () {
            _this._accountService.downloadNewTransactions2([balance.id]);
        }, function (err) {
        });
    ...
    JaxxCryptoController.prototype.checkBalanceCurrentReceive = function () {
        ...
        console.log(this.name + ' checkBalanceCurrentReceive  ' + address + '  ' + new Date().toTimeString());
    ...
    JaxxCryptoController.prototype.checkAllBalances = function (cb, onError) {
        ...
        this.downloadAllBalances(cb, function (err) { });
        return cb;
    };
    ...
    JaxxCryptoController.prototype.generateBalancesFor0Indexes = function () {
        var addressReceive = this.getAddressRecieve(0);
    ...
    JaxxCryptoController.prototype.getTransactionsFromDB = function (coinType) {
        ...
        switch (coinType) {
            ...
            case COIN_TESTNET_ROOTSTOCK:
                trs = this._db.getTransactionsReceive();
                // console.log(trs);
                return trs.map(function (item) {
                    var cost = _this.convertEtherNumber(item.miningFee * price);
                    return new VOTransactionView({
                        toAddress: item.to,
                        to: item.to,
                        addressIndex: _this.getAddressIndexReceive(item.address),
                        confirmations: item.confirmations,
                        blockNumber: item.block,
                        gasCost: cost,
                        gasUsed: item.miningFee,
                        gasPrice: _this._accountService.getMiningFees(),
