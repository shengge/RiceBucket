...
declare class HDWalletPouch {
    ...
    setDataStorageController(datastorage: JaxxCryptoController);
...
class JaxxCryptoController implements IControllerDB{
   ...
   // intervalAllBalancesCheckDefault:number ;
    ...
    intervalCurrentBalanceCheck:number = 5000;
   ...
   // newWallet:boolean;
    constructor(pouch: HDWalletPouch, config:any) {
    ...
    processConfig(config:any):void{
        this.config = config;
        this.options = config.options[this._coinType] || {};
        let defaults:any = config.defaults;
        for(let str in defaults) if(typeof  this.options[str] ==='undefined' ) this.options[str] = defaults[str];
    }
...
/*
    ...
    sendAmountToAddress(address: string, amount: number, addressChange: string): void {
...
/////////////////////////////////////////// ACTIVATE ////////////////////////////
   ...
   /* dispatchInit(): void {
    ...
    getAddressRecieve(i: number): string {
...
/*
    ...
    onCurrentReceiveAddressGotBalance(balance:VOBalance): void {
        ...
        this.downloadAllBalances(()=>{
            ...
            this._accountService.downloadNewTransactions2([balance.id]);
    ...
    checkAllBalances(cb:Function, onError:Function): Function {
        ...
        this.downloadAllBalances(cb, err=>{});
        return cb;
    }
   ...
   downloadingBalancesCounter:number;
    ...
    generateBalancesFor0Indexes():void{
       ...
       let addressReceive:string = this.getAddressRecieve(0);
               ...
               // console.log(trs);
                return trs.map(item => {
                    ...
                    return new VOTransactionView({
                        ...
                        gasPrice: this._accountService.getMiningFees(),
