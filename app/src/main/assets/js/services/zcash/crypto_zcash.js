...
(function (jaxx) {
    var CryptoZCash = (function (_super) {
        ...
        CryptoZCash.prototype.mapInput = function (curInput, referenceTxDetails) {
        ...
        CryptoZCash.prototype.mapOutput = function (curOutput, referenceTxDetails) {
            if (isNaN(referenceTxDetails.time_utc)) {
                referenceTxDetails.time_utc = Math.round(Date.now() / 1000);
            }
            var newVOTransaction = new VOTransaction(null);
        ...
        CryptoZCash.prototype.getVOTransactionsForTxListWithDetails = function (txList) {
            var outAddresses = [];
            var outTransactions = [];
        ...
        CryptoZCash.prototype.getTransactionalDataFromTxList = function (txList) {
            var _this = this;
            var mainRequestDeferred = $.Deferred();
            var txDetailsDataRequest = new jaxx.TransactionDetailsBlockchain(); //this._cryptoMethods['transactionDetails']();
        ...
        CryptoZCash.prototype.downloadTransactions = function (addresses) {
            var _this = this;
            var deffered = $.Deferred();
            var txListRequest = new this._cryptoMethods['transactionList'](this._coin_HD_index, this.generator);
