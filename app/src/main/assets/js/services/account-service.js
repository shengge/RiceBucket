...
(function (jaxx) {
    ...
    var JaxxAccountService = (function () {
        function JaxxAccountService(settings, db, options) {
            this.onTransaction$ = $({});
            //////////////////////////
        ...
        JaxxAccountService.prototype.getMiningFees = function () {
        ...
        JaxxAccountService.prototype.initTokenData = function () {
            ...
            var address = this.crypto_class.generator.generateAddressReceive(0);
            //this._db.saveCurrentIndexReceive(0);
        ...
        JaxxAccountService.prototype._downloadNewTranasctions = function () {
            ...
            if (addresses.length === 0)
                return;
            console.log('%c ' + this.name + '  download New Transactions total addresses: ' + addresses.length, 'color:brown');
        ...
        JaxxAccountService.prototype.onDownloadedNewTransactions = function (transactions) {
            ...
            if (newTransactions.length) {
                ...
                setTimeout(function () { return _this._downloadNewTranasctions(); }, 2000);
            }
        ...
        JaxxAccountService.prototype.downloadBalancesReceive = function (callBack, onError) {
            ...
            this.downloadBalances(addresses).done(function (newbalances) {
                ...
                if (Math.abs(delta) > precision) {
                    ...
                    console.log('%c ' + _this.name + ' same balance receive ' + balanceOld / 1e8, 'color:#f99');
                }
        ...
        JaxxAccountService.prototype.downloadBalancesChange = function (callBack, onError) {
            ...
            this.downloadBalances(addresses).done(function (newbalances) {
                ...
                ;
                var delta = balanceOld - balanceNew;
                var precision = balanceNew / 1e5;
                //console.log(newbalances);
                // console.log(delta + ' ' + precision );
                if (Math.abs(delta) > precision) {
                    console.log('%c ' + _this.name + ' balances change delta more then precision ' + delta + ' precision ' + precision, 'color:red');
                    var balanceOnCurrent = jaxx.Utils.filterBalanceOnAddress(currentAddressChange, difference);
                    ...
                    //this._db.saveBalancesChange(res);
                    console.log('%c' + _this.name + ' same balance change ' + balanceOld / 1e15, 'color:#f99');
                }
