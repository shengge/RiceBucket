...
JaxxUI.prototype.initializeElements = function() {
    ...
    this.generatePaperWalletOptions();
...
JaxxUI.prototype.openShapeShiftSendModal = function() {
    $('.notificationOverlay .cssShapShiftConfirmation').parent().removeClass('cssStartHidden');
    if( $('.tabContent.cssTabContent').hasClass('cssSelected') ) {
        $('.tabSend.tab').trigger('click');
        setTimeout(function () {
            Navigation.openNotificationBanner('.cssShapShiftConfirmation');
        }, 800);
    }
    else {
        Navigation.openNotificationBanner('.cssShapShiftConfirmation');
...
JaxxUI.prototype.showShiftModal = function() {
    if (!$('.modal-bottom .cssShapShiftConfirmation').hasClass('visible')) {
    ...
    if (curCoinType === COIN_BITCOIN) {
        ...
        $('.modal-bottom .cssShapShiftConfirmation .miningFeeSelector').show();
        $('.modal-bottom .cssShapShiftConfirmation .modal-body-bottom-description').show();
        
        if($('.modal-bottom .cssShapShiftConfirmation .mining-fee-title').hasClass('modal-body-bottom-title-description-visible')) {
            return;
        }
        else {
            $('.modal-bottom .cssShapShiftConfirmation .mining-fee-title').addClass('modal-body-bottom-title-description-visible');
            $('.modal-bottom .cssShapShiftConfirmation .mining-fee-title').removeClass('modal-body-bottom-title');
        }
    } else {
        /* $('.modal.shift .miningFeeSelector').hide(); */
        $('.modal-bottom .cssShapShiftConfirmation .miningFeeSelector').hide();
        $('.modal-bottom .cssShapShiftConfirmation .modal-body-bottom-description').hide();
        
        if($('.modal-bottom .cssShapShiftConfirmation .mining-fee-title').hasClass('modal-body-bottom-title')) {
            return;
        }
        else {
            $('.modal-bottom .cssShapShiftConfirmation .mining-fee-title').removeClass('modal-body-bottom-title-description-visible');
            $('.modal-bottom .cssShapShiftConfirmation .mining-fee-title').addClass('modal-body-bottom-title');
        }
    }
    /*
...
JaxxUI.prototype.setPinCirclesToRedBorder = function(pinPadInstance){  
    ...
    $(pinPadInstance + ' input').css('color', '#F45B82 !important');
}
...
JaxxUI.prototype.setPinCirclesToBlueBorder = function(pinPadInstance){
    ...
    $(pinPadInstance + ' input').css('color', '#2681FF !important');
}
...
JaxxUI.prototype.updateAndLoadPrivateKeyList = function(coinType) {

    var self = this;
    this._strKeyPair = "address,privatekey, ";
    if (g_JaxxApp.getDataStoreController().getCryptoControllerByCoinType(coinType).hasIndexes()){
        this.updatePrivateKeyList(coinType);
    } else {
        g_JaxxApp.getDataStoreController().getCryptoControllerByCoinType(coinType).restoreHistoryAll(function(){self.updatePrivateKeyList(coinType)});
    }
}
...
JaxxUI.prototype.updateShapeShiftDisplay = function(coinTypeDict, displayDict) {
    ...
    if ( g_JaxxApp.getShapeShiftHelper().getIsTriggered() && curCoinType === coinTypeDict.send) {
        ...
        if (typeof(displayDict.exchangeRate) !== 'undefined' && displayDict.exchangeRate !== null) {
            ...
            $('.before .shapeShiftAbbreviatedUnitSend').text('1 ' + coinAbbreviatedNameSend);
            $('.shapeShiftAbbreviatedUnitSend').not('.before .shapeShiftAbbreviatedUnitSend').text(coinAbbreviatedNameSend);
...
JaxxUI.prototype.getReleaseBulletin = function(callback) {
    var self = this;
    // **** Reminder:  When testing use different endpoint - not the live one ****
    var url = "https://jaxx.io/jaxx-data/jaxx-release-bulletin-2.php"
    //    var url = "https://jaxx.io/jaxx-data/jaxx-news-beta.php"
    $.getJSON( url, function( data ) {
        if (data && data[0]) {
            self._jaxxReleaseBulletin = data[0];
            callback();
            //self.displayJaxxReleaseBulletinIfUnseen();
        }
    });
}
...
JaxxUI.prototype.displayJaxxReleaseBulletin = function() {
    console.log("news data :: " + JSON.stringify(this._jaxxReleaseBulletin));
...
JaxxUI.prototype.completeSwitchToCoinType = function(currentCoinType, targetCoinType) {
    ...
    jaxx.Registry.application$.triggerHandler(jaxx.Registry.COMPLETE_SWITCH_TO_COIN_TYPE, targetCoinType);
...
JaxxUI.prototype.generateSettingsCryptoCurrencyRows = function() {
    ...
    for (var i = 0; i < cryptoCurrencies.length; i++) {
        ...
        if (g_JaxxApp.getSettings().isCryptoCurrencyEnabled(cryptoCurrencies[i])) {
            ...
            this.enableCryptoCurrencyInUI(cryptoCurrencies[i], true);
        } else {
            g_JaxxApp.getSettings().disableCryptoCurrency(cryptoCurrencies[i]);
            this.disableCryptoCurrencyInUI(cryptoCurrencies[i], true);
        }
    }
...
JaxxUI.prototype.StartFlowSettingsCryptoCurrencyRows = function() {
    // Assertion: Settings has correctly stored the position order of the cryptocurrencies.
    var cryptoCurrencies = g_JaxxApp.getSettings().getCryptoCurrencyPositionList();
    for (var i = 0; i < cryptoCurrencies.length; i++) {
        var cryptoCurrencyName = cryptoCurrencies[i];
        ...
        var column2 = '<td class="itemNumberLabel cssItemNumberLabel"></td>';
        var column3 = '<td class="coinIcon cssCoinIcon"></td>';
        var column4 = '<td class="coinLabel cssCoinLabel">' + cryptoCurrencies[i] + ' - ' + HDWalletHelper.dictCryptoCurrency[cryptoCurrencies[i]]['name'] + '</td>';
        var column5 = '<td class="handle cssHandle"><img src="images/dragAndDrop.svg" alt="" height="13" width="13" style="position:absolute; padding-top:15px;"></td>';
        var tableRow = '<tr class="cssCoinCurrency scriptAction coinType' + cryptoCurrencies[i] + " " + extraCss + '" specialAction ="toggleCryptoCurrency" value="' + cryptoCurrencies[i] + '">' + column1 + column2 + column3 + column4 + column5 + '</tr>';
...
JaxxUI.prototype.removeMouseHoverHandlersFromCoinBanner = function(coinType){
...
JaxxUI.prototype.addListeners = function () {
    ...
    jaxx.Registry.application$.on(jaxx.Registry.ON_BALANCE_RECEIVE_CHANGE,function (ev, obj) {
      console.log(jaxx.Registry.ON_BALANCE_RECEIVE_CHANGE + ' delta ' + obj.delta);
      if(obj.delta > 0) {

          updateWalletUI(obj.coinType);

          console.log(' Banner Payment received  ');
...
JaxxUI.prototype.clickFoxIconBetweenSendReceive = function(){
    ...
    if ($(".tab.send").hasClass("selected")){ // If the send tab is open
        if (g_JaxxApp.getShapeShiftHelper().getIsTriggered()){
            $('.tabContent .address input').val('').trigger('keyup');
            Navigation.toggleTab("send");
        } else {
            // @TODO: Only if tab is toggled.
            g_JaxxApp.getUI().showShapeShift();
            //$(".tabContent .spendable .btnActionShapeShift").trigger("click");
        }        
    } else { // If Send tab is not open.
        Navigation.toggleTab("send");
        g_JaxxApp.getUI().showShapeShift();
    }
}
...
JaxxUI.prototype.loadWalletFromEncryptedMnemonic = function(strMnemonic){
  // // jaxx.Registry.currentCoinType = g_JaxxApp.getSettings().getDefaultCoinType();
    ...
    setTimeout(function(){ loadFromEncryptedMnemonic(strMnemonic, function(err, wallet) { // Timeout because of animation time in Navigation clear settings
        if (err) {
...
JaxxUI.prototype.attachClickEventForScriptAction = function(jquerySelector){
    ...
    $(jquerySelector).click(function (event) { // Add the scriptAction triggers again.
        ///try {
            scriptAction(event);
        //} catch (err) {
            //console.error(err);
...
JaxxUI.prototype.generatePrivateKeyMenuOptions = function(){
    ...
    for (var i = 0 ; i < COIN_NUMCOINTYPES; i++){
        this.addCoinToPrivateKeyListIfMissing(i);
    }
    this.attachClickEventForScriptAction(".settings.backupPrivateKeys .privateKeyMenuList .scriptAction");
}

JaxxUI.prototype.addCoinToPrivateKeyListIfMissing = function(coinType){
    if (HDWalletHelper.isPrivateKeyCryptoCurrencyAllowed(coinType) && this._privateKeysDisplayedInList.indexOf(coinType) === -1) { // @NOTE: isPrivateKeyCryptoCurrencyAllowed(i)
        ...
        $(".settings.backupPrivateKeys .privateKeyMenuList").append(this.getHtmlForPrivateKeyMenuItem(coinType));
...
JaxxUI.prototype.getHtmlForPrivateKeyMenuItem = function(coinType){
    ...
    var coinAbbreviatedName = HDWalletPouch.getStaticCoinPouchImplementation(coinType).pouchParameters.coinAbbreviatedName;
    var htmlElement = '<div class="displayPrivateKeys' + coinAbbreviatedName + ' cssInitialHeight SettingsButton cssExpandableText expandableText expandableDetailsAncestor cssbackupTab scriptAction" specialAction="jaxx_controller.clickDisplayPrivateKeysMenuOption" value="'+coinType+'"' + '>'
...
JaxxUI.prototype.getHtmlForPaperWalletOptions = function(coinAbbreviatedName){
    var expandedHeight = "\'[[355, \"120px\"], [2000, \"100px\"]]\'";
    var strHtmlForPaperWalletOption = '<div class="settingsResetCache expandableText cssExpandableText scriptAction cssOpacity expandableDetailsAncestor cssInitialHeight" pushSettings="paperWarning" specialAction="jaxx_ui.preparePaperWarning" value="'+coinAbbreviatedName+'">'
        + '<div class="expandableDetailsHeader cssExpandableDetailsHeader">'
            + '<div class="triangleArrow cssTriangleArrow scriptAction stopPropagation" specialAction="jaxx_ui.toggleNearbyExpandableDetails"></div>'
            + '<div class="optionTrigger cssOptionTrigger">'
                + '<div class="optionHeading cssOptionHeading">'
                    + '<label>' + 'Transfer ' + coinAbbreviatedName + ' Paper Wallet' + '</label>'
                + '</div>'
            + '</div>'
            + '</div>'
    ...
    + '<p class="cssIntroScreenHeading"> Click this option to import your paper wallet / private key. </p>'
            ...
            + '</div>'
        + '</div>';
    return strHtmlForPaperWalletOption;
}

JaxxUI.prototype.generatePaperWalletOptions = function(){
    // $(".pageDisplayPaperWalletOptions .scrollableMenuList")
    
    // // HDWalletPouch.getStaticCoinPouchImplementation(0).pouchParameters['coinAbbreviatedName']
    var paperWalletsAllowed = HDWalletHelper.getArrayPaperWalletCryptoCurrencyAllowed();
    $(".pageDisplayPaperWalletOptions .scrollableMenuList").empty();
    for (var i = 0; i < paperWalletsAllowed.length; i++) {
        $(".pageDisplayPaperWalletOptions .scrollableMenuList").append(this.getHtmlForPaperWalletOptions(paperWalletsAllowed[i]));
    }
    this.attachClickEventForScriptAction(".pageDisplayPaperWalletOptions .scrollableMenuList .scriptAction");
...
JaxxUI.prototype.preparePrivateKeySweep = function(element) {
    //$('.settings.sweepPrivateKey .sweepNextButton').hide();
    var coinAbbreviatedName = $(element).attr('value');
    var coinType = HDWalletHelper.dictCryptoCurrency[coinAbbreviatedName].index;
    this._transferPaperWalletCoinType = coinType;
    this.setTransferPaperWalletHeader(coinType);
...
JaxxUI.prototype.setTransferPaperWalletHeader = function(coinType){
    ...
    if(coinType === COIN_ETHEREUM || coinType === COIN_ETHEREUM_CLASSIC || coinType === COIN_AUGUR_ETHEREUM) {
        coinNetInfo = "AEM";
    }
...
JaxxUI.prototype.hideApplicationLoadingScreen = function(){
