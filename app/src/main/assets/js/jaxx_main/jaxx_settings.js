var JaxxAppSettings = function() {
	...
	this._cryptoCurrencyAtPosition = [];
...
JaxxAppSettings.prototype.getCryptoCurrencyPositionList = function(){
	return this._cryptoCurrencyAtPosition;
}

JaxxAppSettings.prototype.getCryptoCurrencyAtPosition = function(index) {
	return this._cryptoCurrencyAtPosition[index];
}
...
JaxxAppSettings.prototype.initializeCryptoCurrencyPositionData = function(){
    var self = this;
    ...
    var loadedCryptoPositionData = getStoredData('crypto_currency_position_data', false);
...
JaxxAppSettings.prototype.setCryptoCurrencyPositionData = function(newPositionArray){
	try {
		...
		this.pushCryptoCurrencyPositionDataToStorage();
...
JaxxAppSettings.prototype.getCryptoCurrencyEnabledCount = function() {
	try {
		var count = 0;
		for (var i = 0; i < this._cryptoCurrencyAtPosition.length; i++){
			if (this.isCryptoCurrencyEnabled(this._cryptoCurrencyAtPosition[i])){
				count = count + 1;
			}
		}
		return count;
...
JaxxAppSettings.prototype.getListOfEnabledCryptoCurrencies = function(){
	...
	try {
		...
		for (var i = 0; i < this._cryptoCurrencyAtPosition.length; i++){
			var key = this._cryptoCurrencyAtPosition[i];
...
JaxxAppSettings.prototype.resetJaxxCache = function(){
    ...
    var saveAndRestoreData = {
        ...
        "_jaxxReleaseBulletinVersions": {data: null, isEncrypted: false}
    };
