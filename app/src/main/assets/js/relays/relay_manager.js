...
RelayManager.prototype.initialize = function(relaysImplementation, arrRelayNodes) {
    this._relaysImplementation = relaysImplementation;
    
    this._relaysImplementation.initialize(arrRelayNodes);
...
RelayManager.prototype.setup = function(initializationCallback, passthroughParams) {
    this.relayLog("[ " + this._name + " Setup ] :: setup with " + this._relays.length + " relays");

    var self = this;

    this.fetchBlockHeights(function(resultParams) {
        self.relayLog("[ " + self._name + " ] :: fetchBlockHeights :: " + JSON.stringify(resultParams));
        ...
        self.relayLog("[ " + self._name + " ] :: setup :: Now setting up best relay.");
        self.getPrimaryRelay(function(bestRelay) {
            if (bestRelay !== null) {
                self.relayLog("[ "  + self._name + " ] :: setup :: best relay index set to: " + bestRelay);
                self.setBestRelayIndex(bestRelay);
            } else {
                self.relayLog("btcRelays :: error :: no relays available");
...
RelayManager.prototype.relayLog = function() {
    ...
    if (PlatformUtils.browserChromeCheck() && this._relaysImplementation !== null) {
        var debugColor = "background: black;"
        
        if (this._relaysImplementation._debugColor) {
            debugColor = "color: " + this._relaysImplementation._debugColor;
        }
...
//                        console.log("returning callbackArguments :: " + JSON.stringify(callbackArguments));
					...
					callbackArguments.length = Object.keys(callbackArguments).length;
                    this.relayLog("Arguments in case of permanent failure are: " + JSON.stringify(callbackArguments));
...
RelayManager.prototype.getUTXO = function(address, callback){
	...
	var actionTakenWhenTaskIsNotExecuted = function() {
        self.relayLog("Cannot get unspent transaction outputs.");
	}
